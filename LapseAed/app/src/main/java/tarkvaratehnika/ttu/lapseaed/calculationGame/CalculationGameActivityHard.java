...
public class CalculationGameActivityHard extends Activity {

    public static final String MyPREFERENCES = "MyPrefs";
    SharedPreferences sharedPreferences;
    private ImageView bug1 = null;
    private ImageView bug2 = null;
    private ImageView bug3 = null;
    private ImageView bug4 = null;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        setContentView(R.layout.calc_game_hard);

        score = (TextView) findViewById(R.id.score);
        score.setText("" + scoreTotal);
        bug1 = (ImageView) findViewById(R.id.number1);
        bug2 = (ImageView) findViewById(R.id.number2);
        bug3 = (ImageView) findViewById(R.id.number3);
        bug4 = (ImageView) findViewById(R.id.number4);
        ...
        int num3 = (int) (Math.floor(Math.random() * 6) + 1);
        int num4 = (int) (Math.floor(Math.random() * 6) + 1);
        int correct = num1 + num2 + num3 + num4;
        ...
        for (int i = 0; i < t.length; i++) {
            ...
            if (i == 0) {
                ...
                int ans = (int) Math.floor(Math.random() * 24) + 1;
                while (ans == correct || answers.contains(ans)) {
                    ans = (int) Math.floor(Math.random() * 24) + 1;
                }
                answers.add(ans);
    ...
    public void onCheckAnswer(View button) {
        if (button == correctAns) {
            ...
            prepareNextRound(ans1, ans2, ans3, ans4, bug1, bug2, bug3, bug4);
            scoreTotal++;
            isRight = true;
    ...
    public void saveHighScore(int score) {
        ...
        int highScore = sharedPreferences.getInt(name + "CalcHard", 0);

        if (score > highScore) {
            final SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putInt(name + "CalcHard", score);
            editor.apply();
        }
    }
    ...
    public void prepareNextRound(TextView a, TextView b, TextView c, TextView d, ImageView img1, ImageView img2, ImageView img3, ImageView img4) {
        a.setText("");
        b.setText("");
        c.setText("");
        d.setText("");
        ...
        img3.setImageResource(R.drawable.ladybug);
        img4.setImageResource(R.drawable.ladybug);
    }

    public void onClickExit(View v) {
        saveHighScore(scoreTotal);
        Intent i = new Intent(v.getContext(), CalculationGameHighScoreActivity.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
