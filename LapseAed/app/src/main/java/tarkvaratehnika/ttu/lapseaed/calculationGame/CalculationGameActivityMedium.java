...
public class CalculationGameActivityMedium extends Activity {
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        setContentView(R.layout.calc_game_medium);
        ...
        for (int i = 0; i < t.length; i++) {
            ...
            if (i == 0) {
                ...
                int ans = (int) Math.floor(Math.random() * 18) + 1;
                while (ans == correct || answers.contains(ans)) {
                    ans = (int) Math.floor(Math.random() * 18) + 1;
                }
                answers.add(ans);
    ...
    public void onCheckAnswer(View button) {
        if (button == correctAns) {
            ...
            prepareNextRound(ans1, ans2, ans3, ans4, bug1, bug2, bug3);
            scoreTotal++;
            isRight = true;
    ...
    public void saveHighScore(int score) {
        ...
        int highScore = sharedPreferences.getInt(name + "CalcMed", 0);

        if (score > highScore) {
            final SharedPreferences.Editor editor = sharedPreferences.edit();
            editor.putInt(name + "CalcMed", score);
            editor.apply();
        }
    }
    ...
    public void prepareNextRound(TextView a, TextView b, TextView c, TextView d, ImageView img1, ImageView img2, ImageView img3) {
        a.setText("");
        b.setText("");
        c.setText("");
        d.setText("");
